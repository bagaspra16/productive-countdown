...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children, footer }) => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 z-40 bg-dark-bg/40 backdrop-blur-sm transition-opacity duration-300 ease-in-out ${isAnimatingOut ? 'opacity-0' : 'opacity-100'
          }`}
        aria-hidden="true"
      />
      ...
      <div className="fixed inset-0 z-50 overflow-hidden">
        <div className="flex min-h-full items-center justify-center p-4 text-center">
          <div
            ref={modalRef}
            className={`relative transform overflow-hidden rounded-xl bg-dark-surface/90 border border-dark-accent/30 shadow-xl transition-all duration-300 ease-out w-full sm:max-w-lg text-left ${isAnimatingOut
                ? 'opacity-0 scale-95'
                : 'opacity-100 scale-100'
              } flex flex-col max-h-[80vh] sm:max-h-[85vh]`}
          >
